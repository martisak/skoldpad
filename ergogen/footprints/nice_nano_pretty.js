    ...
    body: p => {
      ...
      const gen_socket_row = (row_num, pin_name_left, pin_name_right, show_via_labels, show_silk_labels) => {
        ...
        if(show_silk_labels == true) {
          socket_row += `
            ...
            (fp_text user ${net_silk_front_left} (at -2.75 ${-12.7 + row_offset_y + (1-p.fontsize)/4}) (layer F.SilkS)
              (effects (font (size ${p.fontsize} ${p.fontsize}) (thickness 0.15) (face "${p.font}")) (justify left))
            )
            (fp_text user ${net_silk_front_right} (at 2.75 ${-12.7 + row_offset_y + (1-p.fontsize)/4}) (layer F.SilkS)
              (effects (font (size ${p.fontsize} ${p.fontsize}) (thickness 0.15) (face "${p.font}")) (justify right))
            )

            ${''/* Silkscreen Labels - Back */}
            (fp_text user ${net_silk_back_left} (at -2.75 ${-12.7 + row_offset_y + (1-p.fontsize)/4} 180) (layer B.SilkS)
              (effects (font (size ${p.fontsize} ${p.fontsize}) (thickness 0.15) (face "${p.font}")) (justify right mirror))
            )
            (fp_text user ${net_silk_back_right} (at 2.75 ${-12.7 + row_offset_y + (1-p.fontsize)/4} 180) (layer B.SilkS)
              (effects (font (size ${p.fontsize} ${p.fontsize}) (thickness 0.15) (face "${p.font}")) (justify left mirror))
            )
          `
        }
      ...
      const instructions = `
          (fp_text user "R. Side - Jumper Here" (at 0 -15.245) (layer F.SilkS)
            (effects (font (size ${p.fontsize} ${p.fontsize}) (thickness 0.15) (face "${p.font}")))
          )
          (fp_text user "L. Side - Jumper Here" (at 0 -15.245) (layer B.SilkS)
            (effects (font (size ${p.fontsize} ${p.fontsize}) (thickness 0.15) (face "${p.font}")) (justify mirror))
          )
    `
